...
if (!$tareaData)
     die("Tarea no encontrada o no está en estado 'realizada'");
...
if (!$guAsignadoId)
     die("El asignado no pertenece al grupo");
...
try {
     ...
     $conn->prepare("INSERT INTO historialgrupousuario (fecha, puntosOtorgados, puntosCanjeados, estadoTarea, grupo_usuario_id, tarea_id_tarea, recompensa_id_recompensa)
          ...
          ->execute([
               ':puntos' => $puntos,
               ':gu_id_admin' => $guAdmin['id_grupo_usuario'], // ← este es el admin logueado
               ':tid' => $id_tarea
          ]);
